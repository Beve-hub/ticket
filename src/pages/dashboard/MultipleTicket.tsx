...
const MultipleTicket = () => {
    ...
    const validate = (): boolean => {
        ...
        if (!formData.ticketAmount || isNaN(Number(formData.ticketAmount))) {
            errors.ticketAmount = 'Please enter a valid number of tickets';
            isValid = false;
        }
  ...
  const fee = 1.23;
  const subtotal = Number(ticketDetails.price) * Number(formData.ticketAmount || 0);
  const totalPrice = subtotal + fee;   
    ...
    return (
        ...
        styles={() => ({
            ...
            <Stepper.Step>
            <Text fz={20} fw={600} > Multiple Ticket </Text>
            <Box mt={10}>
            <Text style={{
                            fontSize: '0.95rem',
                            fontWeight: 'regular'
        ...
        <Box>
                ...
                <Box mt={10} style={{display:'flex', justifyContent: 'space-between'}}>
                    ...
                    <Text fz={15}>{formData.ticketAmount || 0}</Text>
                </Box>
                <Box mt={10} style={{display:'flex', justifyContent: 'space-between'}}>
